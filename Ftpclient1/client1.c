...
printf("Enter 'get' <filename> to download\nEnter 'put' <filename> to upload\nEnter 'quit' to end session\nEnter 'ls' to view the files in the server's directory\nEnter 'pwd' to view the path\nEnter 'mkdir' <dirname> to make new file\nEnter 'cd' <dirname> to change directory\nEnter 'delete' <finename> to delete a file"); 
while (1) 
{ 
...
select = 0; 
//delete, ls, cd, mkdir, pwd and other invalid command 
switch(select) 
{ 
case 1: 
...
printf("%s has been downloaded successfully.\n",f); 
}else 
{ 
printf("1000 bytes of %s has been downloaded successfully.\n",f); 
} 
bzero(r_str, strlen(r_str)); 
} 
fclose(file); 
...
if (file == NULL){ 
printf("No such file to Upload: %s\n", f); 
return; 
} 
write(sid, str, strlen(str));// write the command to the server 
...
if(strlen(buf) < 1000){ 
printf("%s has been sent succesfully.\n", f); 
} 
else 
{ 
printf("1000 bytes of %s has been sent succesfully.\n", f); 
} 
bzero(buf, sizeof(buf)); 
sleep(2); 
} 
