...
//nport is used for data handelling
//tport is used for connection handelling
if (argc != 3)
{
...
void get (int sid, char *str)
{
char *ferror = "No such file present in the directory";// error message
char buf[SIZE];
char f[100];
int i;
char t_id_string[10];
...
char f[100];
int i;
char *fail_delete = "No such file to delete.\n";
char *success_delete = "successfully deleted.\n";
bzero(f, sizeof(f));
// get the file name
...
char *success_cd = "Dir successfully changed to \n";
bzero(f, sizeof(f));
...
char *success_mkdir = " successfully created.\n";
bzero(f, sizeof(f));
